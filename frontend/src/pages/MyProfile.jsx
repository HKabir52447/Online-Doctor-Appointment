  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
            ...
            isEdit ?
              <p>
                <input className='bg-gray-50' onChange={e => setUserData(prev => ({ ...prev, address: { ...prev.address, line1: e.target.value } }))}
                  value={userData.address.line1} /> <br />
      ...
      <div className='mt-10 font-medium'>
        {
          isEdit ?
            <button className='border border-green-500 px-8 py-1 rounded-full hover:bg-green-500 hover:text-white transition-all' onClick={updateUserProfileData}>Save</button>
            :
            <button className='border border-green-500 px-8 py-1 rounded-full hover:bg-green-500 hover:text-white transition-all' onClick={() => setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
